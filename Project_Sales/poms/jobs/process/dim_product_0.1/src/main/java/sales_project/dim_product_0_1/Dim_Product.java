...
package sales_project.dim_product_0_1;
...
public class Dim_Product implements TalendJob {
	...
	public static class dim_productStruct implements routines.system.IPersistableRow<dim_productStruct> {
		final static byte[] commonByteArrayLock_SALES_PROJECT_Dim_Product = new byte[0];
		static byte[] commonByteArray_SALES_PROJECT_Dim_Product = new byte[0];
		...
		public boolean equals(Object obj) {
			...
			if (getClass() != obj.getClass())
				return false;
			final dim_productStruct other = (dim_productStruct) obj;
		...
		public void copyDataTo(dim_productStruct other) {
		...
		public void copyKeysDataTo(dim_productStruct other) {
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_SALES_PROJECT_Dim_Product.length) {
					if (length < 1024 && commonByteArray_SALES_PROJECT_Dim_Product.length == 0) {
						commonByteArray_SALES_PROJECT_Dim_Product = new byte[1024];
					} else {
						commonByteArray_SALES_PROJECT_Dim_Product = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_SALES_PROJECT_Dim_Product, 0, length);
				strReturn = new String(commonByteArray_SALES_PROJECT_Dim_Product, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_SALES_PROJECT_Dim_Product, 0, length);
				strReturn = new String(commonByteArray_SALES_PROJECT_Dim_Product, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_SALES_PROJECT_Dim_Product) {
		 ...
		 * Compare keys
		 */
		public int compareTo(dim_productStruct other) {
	...
	public void tDBInput_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				dim_productStruct dim_product = new dim_productStruct();
				...
				if (execStat) {
					runStat.updateStatOnConnection(resourceMap, iterateId, 0, 0, "dim_product");
				}
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:lujzQha0QZvZrJqdN7cKyYHoQ3pfPreLC9wflHi1y4M=");
...
// # Outputs initialization
				dim_productStruct dim_product_tmp = new dim_productStruct();
// ###############################
				...
				final String decryptedPassword_tDBInput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:GZDkGXLqns3Rz+YrmMhFwMGsyS5I+6w2vlUrTtzPn8c=");
...
// # Output table : 'dim_product'
								dim_product_tmp.product_id = Numeric.sequence("s1", 1, 1);
								dim_product_tmp.product_name = row2.product;
								dim_product = dim_product_tmp;
// ###############################
...
// Start of branch "dim_product"
							if (dim_product != null) {
								...
								pstmt_tDBOutput_1.setInt(1, dim_product.product_id);

								if (dim_product.product_name == null) {
									pstmt_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(2, dim_product.product_name);
								}
...
// ###############################      
				...
				if (execStat) {
					runStat.updateStat(resourceMap, iterateId, 2, 0, "dim_product");
				}
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Dim_Product.class.getClassLoader()
					.getResourceAsStream("sales_project/dim_product_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
...
/************************************************************************************************
 * 80563 characters generated by Talend Open Studio for Data Integration on the
 * 16 novembre 2024 à 23:38:02 WEST
 ************************************************************************************************/